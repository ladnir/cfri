...
use crate::plonkish_backend::util::binius_iter;
use crate::plonkish_backend::util::binius_iter::IterExtensions;
use std::{iter, marker::PhantomData, ops::Deref};

use crate::plonkish_backend::Error;

use crate::plonkish_backend::util::arithmetic::PrimeField;
...
fn basis_checked<F: PrimeField>(i: usize) -> Result<F, Error> {
    ...
    Ok(F::from(1 << (i) as u64))
}

const DEGREE: usize = 128;
const LOGDEGREE: usize = 7;
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BinarySubspace<F: PrimeField> {
    basis: Vec<F>,
}
...
impl<F: PrimeField> BinarySubspace<F> {
    ...
    pub fn get_twiddle_pair(&self, level: usize, index: usize) -> (F, F) {
        let w1 = self.get(index);
        let w2 = self.get(index + (1 << level));
        (w1, w2)
    }
}
...
pub trait TwiddleAccess<F: PrimeField> {
...
fn subset_sum<F: PrimeField>(values: &[F], n_bits: usize, index: usize) -> F {
    (0..n_bits)
...
impl<F: PrimeField> PrecomputedTwiddleAccess<F> {
    pub fn generate(subspace: &BinarySubspace<F>) -> Result<Vec<Self>, Error> {
...
fn precompute_subspace_evals<F: PrimeField>(
...
fn subspace_map<F: PrimeField>(elem: F, constant: F) -> F {
...
mod tests {
use binius_field::{PrimeField, BinaryField16b, BinaryField32b, BinaryField8b};
