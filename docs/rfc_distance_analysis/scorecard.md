...
| Top tau-one/root-kernel carry contract | 0 | 1 | 0 | 0% | Promising but risky | `rfc_top_tau1_root_kernel_carry_block.md` isolates the legal version of the top-row credit: carry the full quotient line, charge descendant lines by `q^kappa_phi`, and cover only unconsumed root-kernel fibers. It also identifies the current broad tau-one root-kernel diagnostic as unsafe until row eligibility, `kappa_phi`, and consumed/unconsumed split are explicit. |
