...
mod test {
    ...
    pub(super) fn run_commit_open_rejects_wrong_eval<F, Pcs, T>()
    where
        F: PrimeField,
        Pcs: PolynomialCommitmentScheme<F>,
        T: TranscriptRead<Pcs::CommitmentChunk, F>
            ...
            + InMemoryTranscript<Param = ()>,
    {
        for k in 2..3 {
            ...
            let proof = {
                ...
                let point = <Pcs::Polynomial as Polynomial<F>>::squeeze_point(k, &mut transcript);
                let eval = poly.evaluate(&point);
                let wrong_eval = eval + F::ONE;
                transcript.write_field_element(&wrong_eval).unwrap();
                Pcs::open(&pp, &poly, &comm, &point, &eval, &mut transcript).unwrap();
                transcript.into_proof()
            };
            ...
            assert!(
                result.is_err(),
                "PCS verifier accepted a wrong claimed evaluation"
            );
        }
    }

    pub(super) fn run_commit_open_rejects_tampered_proof<F, Pcs, T>()
    where
        F: PrimeField,
        Pcs: PolynomialCommitmentScheme<F>,
        T: TranscriptRead<Pcs::CommitmentChunk, F>
            ...
            + InMemoryTranscript<Param = ()>,
    {
        for k in 2..3 {
            ...
            assert!(result.is_err(), "PCS verifier accepted a tampered proof");
        }
    }
    ...
    pub(super) fn run_batch_commit_open_rejects_wrong_eval<F, Pcs, T>()
    where
        F: PrimeField,
        Pcs: PolynomialCommitmentScheme<F>,
        T: TranscriptRead<Pcs::CommitmentChunk, F>
        ...
        for k in 2..3 {
            ...
            let proof = {
                ...
                let true_evals = eval_indices
                    ...
                    .collect_vec();
                let mut wrong_values = true_evals.iter().map(|eval| *eval.value()).collect_vec();
                wrong_values[0] += F::ONE;
                transcript.write_field_elements(&wrong_values).unwrap();
                Pcs::batch_open(&pp, &polys, &comms, &points, &true_evals, &mut transcript)
                    .unwrap();
                transcript.into_proof()
            };
            ...
            let result = {
                ...
                Pcs::batch_verify(
                    ...
                    &eval_indices
                        .iter()
                        .copied()
                        .zip(transcript.read_field_elements(eval_indices.len()).unwrap())
                        .map(|((poly, point), eval)| Evaluation::new(poly, point, eval))
                        .collect_vec(),
                    &mut transcript,
                )
            ...
            assert!(
                result.is_err(),
                "PCS batch verifier accepted a wrong claimed evaluation"
            );
        }
    }
}
