...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(bound(serialize = "F: Serialize", deserialize = "F: DeserializeOwned"))]
pub struct HidingBasefoldParams<F: PrimeField> {
    inner: BasefoldParams<F>,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(bound(serialize = "F: Serialize", deserialize = "F: DeserializeOwned"))]
pub struct HidingBasefoldProverParams<F: PrimeField> {
    inner: BasefoldProverParams<F>,
    public_num_vars: usize,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(bound(serialize = "F: Serialize", deserialize = "F: DeserializeOwned"))]
pub struct HidingBasefoldVerifierParams<F: PrimeField> {
    inner: BasefoldVerifierParams<F>,
    public_num_vars: usize,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(bound(serialize = "F: Serialize", deserialize = "F: DeserializeOwned"))]
pub struct HidingBasefoldCommitment<F: PrimeField, H: Hash> {
    inner: BasefoldCommitment<F, H>,
    hidden_poly: MultilinearPolynomial<F>,
}

impl<F: PrimeField, H: Hash> Default for HidingBasefoldCommitment<F, H> {
    fn default() -> Self {
        Self {
            inner: BasefoldCommitment::default(),
            hidden_poly: MultilinearPolynomial::default(),
        }
    }
}

impl<F: PrimeField, H: Hash> AsRef<[Output<H>]> for HidingBasefoldCommitment<F, H> {
    fn as_ref(&self) -> &[Output<H>] {
        self.inner.as_ref()
    }
}

impl<F: PrimeField, H: Hash> AdditiveCommitment<F> for HidingBasefoldCommitment<F, H> {
        ...
        Self: 'a,
    {
        ...
        let inner = BasefoldCommitment::sum_with_scalar(
            scalars.iter().copied(),
            bases.iter().map(|c| &c.inner),
        );

        let hidden_poly = if bases.iter().all(|comm| !comm.hidden_poly.is_zero()) {
            bases
                .iter()
                .zip(scalars.iter())
                .map(|(comm, scalar)| (**scalar, comm.hidden_poly.clone()))
                .sum()
        } else {
            MultilinearPolynomial::default()
        };

        Self { inner, hidden_poly }
    }
}

impl<F: PrimeField, H: Hash> PartialEq for HidingBasefoldCommitment<F, H> {
    fn eq(&self, other: &Self) -> bool {
        self.inner == other.inner && self.hidden_poly.evals() == other.hidden_poly.evals()
    }
}

impl<F: PrimeField, H: Hash> Eq for HidingBasefoldCommitment<F, H> {}

#[derive(Debug)]
pub struct HidingBasefold<F: PrimeField, H: Hash, V: BasefoldExtParams>(PhantomData<(F, H, V)>);

impl<F: PrimeField, H: Hash, V: BasefoldExtParams> Clone for HidingBasefold<F, H, V> {
    fn clone(&self) -> Self {
        Self(PhantomData)
    }
}

fn hide_multilinear_poly<F: PrimeField>(
    poly: &MultilinearPolynomial<F>,
) -> MultilinearPolynomial<F> {
    let mut rng = ChaCha8Rng::from_entropy();
    let mut evals = Vec::with_capacity(poly.evals().len() * 2);
    evals.extend_from_slice(poly.evals());
    evals.extend(poly.evals().iter().map(|eval| *eval + F::random(&mut rng)));
    MultilinearPolynomial::new(evals)
}

fn extend_hiding_point<F: PrimeField>(point: &[F]) -> Vec<F> {
    let mut hidden_point = Vec::with_capacity(point.len() + 1);
    hidden_point.extend_from_slice(point);
    hidden_point.push(F::ZERO);
    hidden_point
}
...
impl<F, H, V> PolynomialCommitmentScheme<F> for HidingBasefold<F, H, V>
where
    F: PrimeField + Serialize + DeserializeOwned,
    H: Hash,
    V: BasefoldExtParams,
{
    type Param = HidingBasefoldParams<F>;
    type ProverParam = HidingBasefoldProverParams<F>;
    type VerifierParam = HidingBasefoldVerifierParams<F>;
    type Polynomial = MultilinearPolynomial<F>;
    type Commitment = HidingBasefoldCommitment<F, H>;
    type CommitmentChunk = Output<H>;

    fn setup(poly_size: usize, batch_size: usize, rng: impl RngCore) -> Result<Self::Param, Error> {
        let inner = Basefold::<F, H, V>::setup(poly_size << 1, batch_size, rng)?;
        Ok(Self::Param { inner })
    }
    ...
    fn trim(
        ...
        let (pp, vp) = Basefold::<F, H, V>::trim(&param.inner, poly_size << 1, batch_size)?;
        let public_num_vars = log2_strict(poly_size);
        Ok((
            Self::ProverParam {
                inner: pp,
                public_num_vars,
    ...
    fn commit(pp: &Self::ProverParam, poly: &Self::Polynomial) -> Result<Self::Commitment, Error> {
        if poly.num_vars() != pp.public_num_vars {
            return Err(Error::InvalidPcsParam(format!(
                "Invalid hiding BaseFold polynomial: expected {} variates, got {}",
                pp.public_num_vars,
                poly.num_vars()
            )));
        }
        let hidden_poly = hide_multilinear_poly(poly);
        let inner = Basefold::<F, H, V>::commit(&pp.inner, &hidden_poly)?;
        Ok(Self::Commitment { inner, hidden_poly })
    }
    ...
    fn open(
        ...
        if point.len() != pp.public_num_vars {
            return Err(Error::InvalidPcsOpen(format!(
                "Invalid hiding BaseFold opening point: expected {} coordinates, got {}",
                pp.public_num_vars,
                point.len()
            )));
        }
        if comm.hidden_poly.is_zero() {
            return Err(Error::InvalidPcsOpen(
                "Missing hiding BaseFold prover state in commitment".to_string(),
            ));
        }
        let hidden_point = extend_hiding_point(point);
        Basefold::<F, H, V>::open(
            &pp.inner,
            &comm.hidden_poly,
            &comm.inner,
    ...
    fn batch_open<'a>(
        pp: &Self::ProverParam,
        _polys: impl IntoIterator<Item = &'a Self::Polynomial>,
        comms: impl IntoIterator<Item = &'a Self::Commitment>,
        points: &[Point<F, Self::Polynomial>],
        evals: &[Evaluation<F>],
        transcript: &mut impl TranscriptWrite<Self::CommitmentChunk, F>,
        ...
        Self::Commitment: 'a,
    {
        ...
        let comms = comms.into_iter().collect_vec();
        if comms.iter().any(|comm| comm.hidden_poly.is_zero()) {
            return Err(Error::InvalidPcsOpen(
                "Missing hiding BaseFold prover state in commitment".to_string(),
            ));
        }
        let hidden_points = points
            .iter()
            .map(|point| extend_hiding_point(point))
            .collect_vec();
        Basefold::<F, H, V>::batch_open(
            &pp.inner,
            comms.iter().map(|comm| &comm.hidden_poly),
            comms.iter().map(|comm| &comm.inner),
            &hidden_points,
            evals,
            transcript,
        )
    ...
    fn read_commitments(
        ...
        let inner = Basefold::<F, H, V>::read_commitments(&vp.inner, num_polys, transcript)?;
        Ok(inner
            .into_iter()
            .map(|inner| Self::Commitment {
                inner,
                hidden_poly: MultilinearPolynomial::default(),
            })
            .collect())
    ...
    fn verify(
        ...
        if point.len() != vp.public_num_vars {
            return Err(Error::InvalidPcsOpen(format!(
                "Invalid hiding BaseFold opening point: expected {} coordinates, got {}",
                vp.public_num_vars,
                point.len()
        ...
        let hidden_point = extend_hiding_point(point);
        Basefold::<F, H, V>::verify(&vp.inner, &comm.inner, &hidden_point, eval, transcript)
    }
...
fn bench_multilinear_eval() {
    use crate::backend::ff_255::ff255::Ft255;
    for i in 10..27 {
        let mut rng = ChaCha8Rng::from_entropy();
...
mod test {
    ...
    use crate::backend::{
        ...
        new_fields::{Mersenne127, Mersenne61},
        pcs::{
            multilinear::test::{run_batch_commit_open_verify, run_commit_open_verify},
    ...
    use crate::plonky2_util::reverse_index_bits_in_place;
    ...
    use crate::backend::arithmetic::PrimeField;
    use crate::backend::basefold::BasefoldExtParams;
    use crate::backend::basefold::Instant;
    ...
    fn test_transforms() {
        ...
        let mut commitment = evaluate_over_foldable_domain::<F>(
            log_rate,
            coeffs.clone(),
            &table,
            "random".to_string(),
    ...
    fn test_binary_rs_transform() {
        ...
        for num_vars in 1..=5 {
            for log_rate in 1..=2 {
                ...
                let commitment = evaluate_over_foldable_domain::<F>(
                    log_rate,
                    coeffs.clone(),
                    &table,
                    "binary_rs".to_string(),
                ...
                let commitment_after = evaluate_over_foldable_domain::<F>(
                    log_rate,
                    coeffs,
                    &table,
                    "binary_rs".to_string(),
                );
                let left_commitment = evaluate_over_foldable_domain::<F>(
                    log_rate,
                    left_coeffs,
                    &table,
                    "binary_rs".to_string(),
                );
                let right_commitment = evaluate_over_foldable_domain::<F>(
                    log_rate,
                    right_coeffs,
                    &table,
                    "binary_rs".to_string(),
    ...
    fn test_binary_rs_table_weights() {
        ...
        for num_vars in 1..=5 {
            for log_rate in 1..=2 {
                ...
                let subspace = crate::backend::code::binary_rs::BinarySubspace::<F>::with_dim(
                    num_vars + log_rate + 1,
                )
                .unwrap();
                ...
                for level in 0..table_w_weights.len() {
                    for index in 0..table_w_weights[level].len() {
                        let (x0, x1) = super::query_binary_table::<F>(
                            &table_w_weights,
                            level,
                            index,
                            &subspace,
