...
its soundness error, by testing that the submitted word is close to
...
to it; it lets the prover submit a claimed child oracle.  The query phase is
...
\begin{enumerate}[leftmargin=0.24in,label=\textbf{P\arabic*.},itemsep=0.08em,topsep=0.1em,parsep=0pt]
  \item \(\prover\): \(R_d\gets\mathcal F_{\mathrm{MsgOracle}}.\mathsf{Submit}(\pi_d)\).
  ...
  \item For \(i=d-1,d-2,\ldots,0\):
        \begin{enumerate}[leftmargin=0.28in,label=\textbf{P\arabic*.\alph*.},itemsep=0.05em,topsep=0.05em,parsep=0pt]
          ...
          \item \(\prover\): choose a claimed child oracle
                \(\pi_i\in\F^{\B^{n_i}}\).
          \item \(\prover\): \(R_i\gets\mathcal F_{\mathrm{MsgOracle}}.\mathsf{Submit}(\pi_i)\).
...
the full tensors that the prover submitted to the functionality.  In
particular, after an accepting execution the extractor can inspect the submitted
top oracle
$\pi_d\in\F^{\B^{n_d}}$.
...
\begin{description}[leftmargin=0.2in,style=nextline,itemsep=0.16em,topsep=0.2em,parsep=0pt]
  ...
  \item[Commit phase.]
        \begin{enumerate}[leftmargin=0.28in,label=\textbf{C\arabic*.},itemsep=0.08em,topsep=0.1em,parsep=0pt]
          ...
          \item \(\prover\): \(C_d\gets\mathcal F_{\mathrm{MsgOracle}}.\mathsf{Submit}(\pi_d)\).
          ...
          \item For $i=d-1,d-2,\ldots,0$:
            \begin{enumerate}[leftmargin=0.24in,label=\textbf{\alph*.},itemsep=0.06em,topsep=0.06em,parsep=0pt]
              ...
              \item \(\prover\): for every \(a\in\B^{n_i}\), compute
                ...
                \]
              \item \(\prover\): \(C_i\gets\mathcal F_{\mathrm{MsgOracle}}.\mathsf{Submit}(\pi_i)\).\\
