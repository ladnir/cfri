...
| Block-credit potential probe | 0 | 1 | 0 | 0% | Promising locator, not proof | `--credit-profile local-incidence` moves the bottleneck from support-three to `top_tau1_to_2_15` and gives support-three `1.06800847` qdims of slack. The old `current-target` ceiling returns the tight row to support-three at `level_weight=1.93994737`, but the audited profile without top line-carry has `level_weight=2.00795584` and remains tight at the top row. This supports the block-grammar direction only if top kernel-fiber cover and support-three incidence become theorem-grade. |
| Top tau-one/root-kernel carry contract | 0 | 1 | 0 | 0% | Narrowed after audit | `rfc_top_tau1_root_kernel_carry_block.md` isolates the legal version of line carry, but `rfc_tau1_carry_kappa_audit.py` shows `top_tau1_to_2_15` has no descendant tau-one line and parent `charged_postroot_qdim=-1`; do not spend `tau1_full_line_carry` on that edge. The live top target is now consumed/unconsumed root-kernel fiber cover. |
