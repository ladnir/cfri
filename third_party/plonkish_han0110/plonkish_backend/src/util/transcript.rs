...
impl<F: PrimeField, R: io::Read> TranscriptRead<Output<Keccak256>, F> for Keccak256Transcript<R> {
    fn read_commitment(&mut self) -> Result<Output<Keccak256>, Error> {
        ...
        self.stream
            ...
            .map_err(|err| Error::Transcript(err.kind(), err.to_string()))?;
        <Self as Transcript<Output<Keccak256>, F>>::common_commitment(self, &hash)?;
        Ok(hash)
    }
}
...
#[cfg(test)]
mod test {
    use crate::util::hash::{Keccak256, Output};
    use halo2_curves::bn256::Fr;
    use std::io::Cursor;

    use super::{
        FieldTranscript, InMemoryTranscript, Keccak256Transcript, TranscriptRead, TranscriptWrite,
    };

    fn challenge_after_hash_commitment(comm: &Output<Keccak256>) -> Fr {
        let mut transcript: Keccak256Transcript<Cursor<Vec<u8>>> = Keccak256Transcript::new(());
        <Keccak256Transcript<Cursor<Vec<u8>>> as TranscriptWrite<Output<Keccak256>, Fr>>::write_commitment(&mut transcript, comm).unwrap();
        <Keccak256Transcript<Cursor<Vec<u8>>> as FieldTranscript<Fr>>::squeeze_challenge(
            &mut transcript,
        )
    }

    #[test]
    fn hash_commitments_are_absorbed_into_challenges() {
        let comm_a = Output::<Keccak256>::default();
        let mut comm_b = Output::<Keccak256>::default();
        comm_b[0] = 1;

        assert_ne!(
            challenge_after_hash_commitment(&comm_a),
            challenge_after_hash_commitment(&comm_b)
        );
    }

    #[test]
    fn hash_commitment_read_write_absorb_consistently() {
        let mut comm = Output::<Keccak256>::default();
        comm[0] = 42;

        let (proof, prover_challenge) = {
            let mut transcript: Keccak256Transcript<Cursor<Vec<u8>>> = Keccak256Transcript::new(());
            <Keccak256Transcript<Cursor<Vec<u8>>> as TranscriptWrite<Output<Keccak256>, Fr>>::write_commitment(&mut transcript, &comm).unwrap();
            let challenge =
                <Keccak256Transcript<Cursor<Vec<u8>>> as FieldTranscript<Fr>>::squeeze_challenge(
            ...
            (transcript.into_proof(), challenge)
        };

        let (read_comm, verifier_challenge) = {
            let mut transcript: Keccak256Transcript<Cursor<Vec<u8>>> =
                Keccak256Transcript::from_proof((), proof.as_slice());
            let read_comm = <Keccak256Transcript<Cursor<Vec<u8>>> as TranscriptRead<
                Output<Keccak256>,
                Fr,
            >>::read_commitment(&mut transcript)
            .unwrap();
            let challenge =
                <Keccak256Transcript<Cursor<Vec<u8>>> as FieldTranscript<Fr>>::squeeze_challenge(
                    &mut transcript,
