...
mod test {
    ...
    macro_rules! tests {
        ($suffix:ident, $pcs:ty, $num_vars_range:expr) => {
            paste::paste! {
                #[test]
                #[ignore = "proof-system matrix test; default tests should stay under one second"]
        ...
        ($suffix:ident, $pcs:ty) => {
            tests!($suffix, $pcs, 2..5);
        };
    }

    tests!(brakedown, MultilinearBrakedown<bn256::Fr, Keccak256, BrakedownSpec6>);
    tests!(hyrax, MultilinearHyrax<grumpkin::G1Affine>, 5..6);
    ...
    type FastBackend = HyperPlonk<MultilinearIpa<grumpkin::G1Affine>>;

    #[test]
    fn vanilla_plonk_w_ipa_rejects_bad_instance() {
        let (circuit_info, circuit) =
            rand_vanilla_plonk_circuit::<_, BinaryField>(2, seeded_std_rng(), seeded_std_rng());
        let mut instances = circuit.instances().to_vec();
        let param = FastBackend::setup(&circuit_info, seeded_std_rng()).unwrap();
        let (pp, vp) = FastBackend::preprocess(&param, &circuit_info).unwrap();
        let proof = {
            let mut transcript = Keccak256Transcript::new(());
            FastBackend::prove(&pp, &circuit, &mut transcript, seeded_std_rng()).unwrap();
            transcript.into_proof()
        };

        instances[0][0] += <grumpkin::G1Affine as CurveAffine>::ScalarExt::ONE;
        let mut transcript = Keccak256Transcript::from_proof((), proof.as_slice());
        let result = FastBackend::verify(&vp, &instances, &mut transcript, seeded_std_rng());
        assert!(
            result.is_err(),
            "HyperPlonk verifier accepted a bad public instance"
        );
    }

    #[test]
    fn vanilla_plonk_w_ipa_rejects_tampered_proof() {
        let (circuit_info, circuit) =
            rand_vanilla_plonk_circuit::<_, BinaryField>(2, seeded_std_rng(), seeded_std_rng());
        let instances = circuit.instances();
        let param = FastBackend::setup(&circuit_info, seeded_std_rng()).unwrap();
        ...
        assert!(
            result.is_err(),
            "HyperPlonk verifier accepted a tampered proof"
        );
    }
}
