...
pub use crate::backend::timer::{end_timer, start_timer, start_unit_timer};
...
macro_rules! izip_eq {
    ...
    (@closure $p:pat => ($($tup:tt)*) , $_iter:expr $(, $tail:expr)*) => {
        $crate::backend::izip_eq!(@closure ($p, b) => ($($tup)*, b) $(, $tail)*)
    };
    ($first:expr $(,)*) => {
        itertools::__std_iter::IntoIterator::into_iter($first)
    };
    ($first:expr, $second:expr $(,)*) => {
        $crate::backend::izip_eq!($first).zip_eq($second)
    };
    ($first:expr $(, $rest:expr)* $(,)*) => {
        $crate::backend::izip_eq!($first)
            $(.zip_eq($rest))*
            .map($crate::backend::izip_eq!(@closure a => (a) $(, $rest)*))
    };
}
