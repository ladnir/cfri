...
```text
docs/rfc_distance_analysis/rfc_column_tensor_structure.md
```

The current certificate theorem outline is:

```text
docs/rfc_distance_analysis/rfc_rank_certificate_outline.md
```

Small bad-shape examples can be decoded with:

```text
python scripts/rfc_distance_analysis/describe_rfc_columns.py --systematic --depth 3 --parity-expansion 7 --columns 0:1:2:3:4:5:8:22:36
```
...
```text
python scripts/rfc_distance_analysis/sample_rfc_rank_failure.py \
  --prime 5 \
  --depth 3 \
  --total-expansion 8 \
  --zero-count 47 \
...
```text
python scripts/rfc_distance_analysis/zero_set_union_from_spectrum.py \
  docs/rfc_distance_analysis/sample_product_first_moment_gf5_depth3_c8_cpp.csv \
  --total-n 64 \
  --zero-count 47
```
...
python scripts/resample_rank_shapes.py \
  ...
  --out docs/rfc_distance_analysis/rank_resample_shapes_systematic_p65537_depth3_c8.csv
```
...
`docs/rfc_distance_analysis/rank_signature_row_*`. The 35 disappearing cases are best treated as accidental zeros of
