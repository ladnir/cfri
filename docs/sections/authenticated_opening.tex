...
The submitted object carries its own natural address space.  If that address
...
\smallskip
\noindent\(\underline{\Pi_{\mathrm{MerkleMsgOracle}}.\mathsf{Submit}(\prover:x\in\mathcal L^{\B^d},\ \verifier:\bot)\to R}\)
...
\(\Pi_{\mathrm{rowFoldCommit}}\) and \(\mathsf{CompositeCommit}\) use
...
\(\Pi_{\mathrm{rowFoldCommit}}\), which folds the committed row-word matrix into
...
\(\Pi_{\mathrm{rowFoldCommit}}.\mathsf{Commit}(R_c,\rho)\) returns a virtual
