...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct BaseFoldTerminalCoreLayerProof<H: Hash> {
    pub authentication_nodes: Vec<Output<H>>,
}

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct BaseFoldTerminalCoreAuthentication<H: Hash> {
    pub folded_layers: Vec<AuxiliaryOraclePublicCommitment<H>>,
    pub layer_authentication: Vec<BaseFoldTerminalCoreLayerProof<H>>,
}

pub type RaaSection5RelationTerminalLayerProof<H> = BaseFoldTerminalCoreLayerProof<H>;

impl<H: Hash> BaseFoldTerminalCoreAuthentication<H> {
...
pub struct BackendProofOracleAuthentication<H: Hash> {
    ...
    pub section5_relation_terminal_core: BaseFoldTerminalCoreAuthentication<H>,
...
pub struct HolographicQuerySchedule {
    ...
    section5_permutation_helper_authentication_queries: Vec<BackendProofQuery>,
}
...
impl Blaze2BaseFoldBackendParams {
    ...
    pub fn open_query_proof<H: Hash>(
        ...
        let section5_permutation_helper = match &state.section5_permutation_helper {
            Some(helper) => {
                Some(helper.prove_schedule(schedule, section5_relation_residual.as_ref())?)
            }
            None => {
        ...
        let mut section5_relation_terminal_core =
            BaseFoldTerminalCoreAuthentication::<H>::default();
        if schedule.raa_relation_strategy() == RaaRelationProofStrategy::Section5 {
            ...
            if let (Some(residual_commitment), Some(residual_proof)) = (
                ...
                let (terminal_proof, terminal_core) = prove_section5_relation_terminal_proof::<H>(
                    &checks,
                    residual_commitment,
                    schedule,
                )?;
                residual_proof.terminal_proof = Some(terminal_proof);
                section5_relation_terminal_core = terminal_core;
            }
        ...
        authentication.section5_relation_terminal_core = section5_relation_terminal_core;
    ...
    pub fn verify_query_proof<H: Hash>(
        ...
        let section5_relation_challenges = squeeze_raa_section5_relation_challenges(
            self.spec(),
            &prequery.compiler_parity,
            prequery.auxiliary.as_ref(),
            request,
        );
        verify_section5_permutation_helper_residual_bindings(
            prequery.section5_permutation_helper.as_ref(),
            prequery.section5_relation_residual.as_ref(),
            proof.section5_permutation_helper.as_ref(),
            proof.section5_relation_residual.as_ref(),
            schedule,
            section5_relation_challenges.gamma,
        )?;
    ...
    fn open_backend_query_set_authentication<H: Hash>(
        ...
        Ok(BackendProofOracleAuthentication {
            ...
            section5_relation_terminal_core: BaseFoldTerminalCoreAuthentication::default(),
...
impl<H: Hash> Blaze2BaseFoldQueryProof<H> {
    pub fn collect_backend_query_set(
        ...
        let section5_permutation_helper_queries = match &self.section5_permutation_helper {
            Some(helper) => helper.authentication_queries(
                schedule,
            ...
            None => {
                let expected_count = section5_permutation_helper_absent_expected_count(
                    schedule,
                    self.section5_relation_residual.as_ref(),
                );
                if expected_count != 0 {
...
impl<H: Hash> BackendProofOracleAuthentication<H> {
    pub fn hash_node_count(&self) -> usize {
        self.compiler_parity_layers
            ...
            + self.section5_relation_terminal_core.hash_node_count()
...
impl<H: Hash> RaaSection5PermutationHelperQueryProof<H> {
    ...
    fn verify_schedule(
        ...
        residual_public: Option<&RaaSection5RelationResidualPublicCommitment<H>>,
...
fn section5_permutation_helper_expected_query_count<H: Hash>(
    helper_public: &RaaSection5PermutationHelperPublicCommitment<H>,
    residual_public: Option<&RaaSection5RelationResidualPublicCommitment<H>>,
    residual_proof: Option<&RaaSection5RelationResidualQueryProof<H>>,
    schedule: &HolographicQuerySchedule,
) -> Result<usize, Error> {
    let len = section5_permutation_residual_len_from_publics(helper_public, residual_public)?;
    let scheduled = build_section5_permutation_helper_authentication_queries(
        len * RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT,
        schedule.proof_queries(),
    )?;
    if scheduled != schedule.section5_permutation_helper_authentication_queries() {
        return Err(Error::InvalidPcsOpen(
            "Section 5 helper authentication schedule does not match residual queries".to_string(),
        ));
    }
    if let Some(residual_proof) = residual_proof {
        let expected_residual_queries = schedule
            .proof_queries()
            .iter()
            .filter(|query| query.domain == BackendProofQueryDomain::Section5RelationResidual);
        for (expected, actual) in expected_residual_queries.zip(residual_proof.queries.iter()) {
            if actual.logical_index != expected.index {
                return Err(Error::InvalidPcsOpen(
                    "Section 5 residual query proof does not match schedule".to_string(),
                ));
            }
        }
    }
...
fn section5_permutation_residual_len_from_helper_len(helper_len: usize) -> Result<usize, Error> {
    if helper_len == 0 || helper_len % RAA_SECTION5_PERMUTATION_HELPER_ROW_COUNT != 0 {
        return Err(Error::InvalidPcsOpen(
            "Section 5 helper oracle length is not row-aligned".to_string(),
        ));
    }
    let len = helper_len / RAA_SECTION5_PERMUTATION_HELPER_ROW_COUNT;
    if !len.is_power_of_two() {
        return Err(Error::InvalidPcsOpen(
            "Section 5 helper oracle domain length must be a power of two".to_string(),
        ));
    }
    Ok(len)
}

fn section5_permutation_residual_len_from_publics<H: Hash>(
    helper_public: &RaaSection5PermutationHelperPublicCommitment<H>,
    residual_public: Option<&RaaSection5RelationResidualPublicCommitment<H>>,
) -> Result<usize, Error> {
    let helper_len = section5_permutation_residual_len_from_helper_len(helper_public.len)?;
    let residual_len = section5_permutation_residual_len_from_residual_public(residual_public)?;
    if residual_len != helper_len {
        return Err(Error::InvalidPcsOpen(
            "Section 5 residual and helper domains have inconsistent lengths".to_string(),
        ));
    }
    Ok(helper_len)
}

fn section5_permutation_residual_len_from_residual_public<H: Hash>(
    residual_public: Option<&RaaSection5RelationResidualPublicCommitment<H>>,
) -> Result<usize, Error> {
    let residual_public = residual_public.ok_or_else(|| {
        Error::InvalidPcsOpen(
            "Section 5 helper local openings require a residual commitment".to_string(),
        )
    })?;
    if residual_public.len == 0
        || residual_public.len % RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT != 0
    {
        return Err(Error::InvalidPcsOpen(
            "Section 5 residual oracle length is not row-aligned".to_string(),
        ));
    }
    let len = residual_public.len / RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT;
    if !len.is_power_of_two() {
        return Err(Error::InvalidPcsOpen(
            "Section 5 residual oracle domain length must be a power of two".to_string(),
        ));
    }
    Ok(len)
}

fn section5_permutation_helper_absent_expected_count<H: Hash>(
    schedule: &HolographicQuerySchedule,
    residual_proof: Option<&RaaSection5RelationResidualQueryProof<H>>,
) -> usize {
    let _ = residual_proof;
...
impl RaaSection5RelationProof {
    ...
    fn verify_openings<H: Hash>(
        ...
        terminal_proof.verify(
            ...
            residual_public,
            schedule,
            &authentication.section5_relation_terminal_core,
        )?;
...
impl HolographicQuerySchedule {
    pub fn sample<H: Hash, S>(
        ...
        let section5_permutation_helper_authentication_queries =
            build_section5_permutation_helper_authentication_queries(
                section5_residual_len,
                &proof_queries,
            )?;
    ...
    pub fn section5_permutation_helper_authentication_queries(&self) -> &[BackendProofQuery] {
        &self.section5_permutation_helper_authentication_queries
    }
    ...
    pub fn section5_permutation_helper_authentication_query_count(&self) -> usize {
        self.section5_permutation_helper_authentication_queries
            .len()
    }
    ...
    pub fn section5_permutation_helper_query_proof_count(&self) -> usize {
        self.section5_permutation_helper_proof_query_count()
            + self.section5_permutation_helper_authentication_query_count()
    }
...
fn raa_section5_permutation_residual_helper_indices(
    len: usize,
    residual_index: usize,
) -> Result<Vec<usize>, Error> {
    if len == 0 || !len.is_power_of_two() || residual_index >= len {
    ...
    let f2_offset = tree_chunk_len * 2;
    let g2_offset = tree_chunk_len * 3;
    let offsets = [f1_offset, g1_offset, f2_offset, g2_offset];

    if residual_index + 1 == len {
        let root_index = product_tree_packed_root_index(len);
        let padding_index = tree_chunk_len - 1;
        return Ok(vec![
            offsets[0] + root_index,
            offsets[1] + root_index,
            offsets[1] + padding_index,
            offsets[2] + root_index,
            offsets[3] + root_index,
            offsets[3] + padding_index,
        ]);
    }
    ...
    for offset in offsets {
        indices.extend(
            product_tree_packed_relation_local_query_indices(len, residual_index)?
                .into_iter()
...
fn raa_section5_permutation_residual_from_helper_openings(
    len: usize,
    gamma: B128,
    residual_index: usize,
    helper_openings: &[(usize, B128)],
) -> Result<B128, Error> {
    let expected_indices = raa_section5_permutation_residual_helper_indices(len, residual_index)?;
    if helper_openings.len() != expected_indices.len() {
        return Err(Error::InvalidPcsOpen(
            "RAA Section 5 helper opening count does not match residual local query".to_string(),
        ));
    }
    for ((actual_index, _), expected_index) in helper_openings.iter().zip(expected_indices.iter()) {
        if actual_index != expected_index {
            return Err(Error::InvalidPcsOpen(
                "RAA Section 5 helper opening index does not match residual local query"
                    .to_string(),
            ));
        }
    }
    ...
    if residual_index + 1 == len {
        let f1_root = helper_openings[0].1;
        let g1_root = helper_openings[1].1;
        let g1_padding = helper_openings[2].1;
        let f2_root = helper_openings[3].1;
        let g2_root = helper_openings[4].1;
        let g2_padding = helper_openings[5].1;
        let gamma2 = gamma * gamma;
        let gamma3 = gamma2 * gamma;
        return Ok(f1_root - g1_root
            + gamma * g1_padding
            + gamma2 * (f2_root - g2_root)
            + gamma3 * g2_padding);
    }
    ...
    let mut residual = B128::ZERO;
    let mut coefficient = B128::ONE;
    for chunk in helper_openings.chunks_exact(3) {
        let left = chunk[0].1;
        let right = chunk[1].1;
        let parent = chunk[2].1;
        residual += coefficient * (parent - left * right);
        coefficient *= gamma;
    }
    Ok(residual)
}
...
fn product_tree_packed_relation_local_query_indices(
    leaf_len: usize,
    relation_index: usize,
) -> Result<Vec<usize>, Error> {
    if leaf_len == 0 || !leaf_len.is_power_of_two() || relation_index >= leaf_len {
        return Err(Error::InvalidPcsOpen(
            "RAA Section 5 product-tree relation query index is invalid".to_string(),
        ));
    }
    if relation_index + 1 == leaf_len {
        return Ok(vec![(leaf_len << 1) - 1]);
    }
    ...
    while level_len > 1 {
        ...
        if relation_index < relation_offset + parent_len {
            ...
            let right_index = left_index + 1;
            let parent_index = parent_offset + local_index;
            return Ok(vec![left_index, right_index, parent_index]);
        }
        relation_offset += parent_len;
        level_offset = parent_offset;
        parent_offset += parent_len;
...
fn prove_section5_relation_terminal_proof<H: Hash>(
    ...
    (
        ...
        BaseFoldTerminalCoreAuthentication<H>,
    ),
    Error,
> {
    ...
    let mut opening_map = BTreeMap::new();
    let mut rows = Vec::with_capacity(row_inputs.len());
    for ((row_index, check, row), folded_values) in row_inputs.iter().zip(row_folded_values.iter())
    {
        rows.push(prove_section5_relation_terminal_row_proof::<H>(
            *row_index,
            check,
            &public,
            row,
    ...
    for (round_offset, commitment) in folded_commitments.iter().enumerate() {
        ...
        folded_layer_authentication.push(BaseFoldTerminalCoreLayerProof {
    ...
    Ok((
        RaaSection5RelationTerminalProof { rows, openings },
        BaseFoldTerminalCoreAuthentication {
            folded_layers,
            layer_authentication: folded_layer_authentication,
        },
    ))
}
...
fn prove_section5_relation_terminal_row_proof<H: Hash>(
    ...
    domain_len: usize,
    query_count: usize,
    openings: &mut BTreeMap<RaaSection5RelationTerminalValueKey, B128>,
...
struct BaseFoldTerminalCoreValueKey {
    round: usize,
    row_index: usize,
    index: usize,
}

type RaaSection5RelationTerminalValueKey = BaseFoldTerminalCoreValueKey;

fn record_section5_relation_terminal_opening(
    openings: &mut BTreeMap<RaaSection5RelationTerminalValueKey, B128>,
    key: RaaSection5RelationTerminalValueKey,
    value: B128,
) -> Result<(), Error> {
    if let Some(existing) = openings.insert(key, value) {
        if existing != value {
            return Err(Error::InvalidPcsOpen(
                "duplicate RAA Section 5 terminal value has conflicting values".to_string(),
            ));
        }
    }
    Ok(())
...
fn section5_relation_terminal_openings_from_map(
    openings: BTreeMap<RaaSection5RelationTerminalValueKey, B128>,
) -> Vec<RaaSection5RelationTerminalValueOpening> {
    openings
        .into_iter()
        .map(|(key, value)| RaaSection5RelationTerminalValueOpening {
            round: key.round,
...
fn section5_relation_terminal_path(
    ...
    if challenges.is_empty() {
        record_section5_relation_terminal_opening(
            openings,
            RaaSection5RelationTerminalValueKey {
                round: 0,
                row_index,
                index: top_index,
            },
...
impl<H: Hash> BaseFoldTerminalCoreProof<H> {
    ...
    fn unique_opening_map(
        &self,
        domain_len: usize,
    ) -> Result<BTreeMap<RaaSection5RelationTerminalValueKey, B128>, Error> {
        let num_vars = log2_strict(domain_len);
        let mut openings = BTreeMap::new();
        for opening in &self.openings {
            let max_round = num_vars.saturating_sub(1);
            if opening.row_index >= RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT
                || opening.round > max_round
            {
                return Err(Error::InvalidPcsOpen(
                    "RAA Section 5 terminal opening address is invalid".to_string(),
                ));
            }
            let active_len = domain_len >> opening.round;
            if active_len == 0 || opening.index >= active_len {
                return Err(Error::InvalidPcsOpen(
                    "RAA Section 5 terminal opening index is outside the layer".to_string(),
                ));
            }
            let key = RaaSection5RelationTerminalValueKey {
                round: opening.round,
                row_index: opening.row_index,
                index: opening.index,
            };
            if let Some(existing) = openings.insert(key, opening.value) {
                if existing != opening.value {
                    return Err(Error::InvalidPcsOpen(
                        "duplicate RAA Section 5 terminal opening has conflicting values"
                            .to_string(),
                    ));
                }
                return Err(Error::InvalidPcsOpen(
                    "duplicate RAA Section 5 terminal opening is not canonical".to_string(),
                ));
            }
        }
        Ok(openings)
    ...
    fn residual_authentication_queries(
        ...
        let openings = self.unique_opening_map(domain_len)?;
        let mut queries = Vec::new();
    ...
    fn verify(
        ...
        public: &RaaSection5RelationResidualPublicCommitment<H>,
        schedule: &HolographicQuerySchedule,
        authentication: &BaseFoldTerminalCoreAuthentication<H>,
    ) -> Result<(), Error> {
        let domain_len = section5_relation_terminal_domain_len(public.len)?;
        let query_count = section5_relation_terminal_query_count(schedule);
        let folded_layers = &authentication.folded_layers;
        let folded_layer_authentication = &authentication.layer_authentication;
        if self.rows.len() != RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT {
            return Err(Error::InvalidPcsOpen(
                "RAA Section 5 terminal proof row count is invalid".to_string(),
            ));
        ...
        let openings = self.unique_opening_map(domain_len)?;
        let mut required_openings = BTreeMap::new();
        ...
        if required_openings.len() != openings.len()
            || !openings
                .keys()
                .all(|key| required_openings.contains_key(key))
        {
            return Err(Error::InvalidPcsOpen(
                "RAA Section 5 terminal proof has non-canonical opening values".to_string(),
            ));
        }
...
impl<H: Hash> BaseFoldTerminalCoreRowProof<H> {
    fn verify(
        ...
        for (path, expected_index) in self.paths.iter().zip(expected_indices.iter().copied()) {
            verify_basefold_terminal_core_path(
                path,
                self.source,
                &self.challenges,
                self.claim,
...
fn verify_basefold_terminal_core_path(
    path: &BaseFoldTerminalCorePath,
    source: BaseFoldTerminalCoreRowSource,
    challenges: &[B128],
    claim: B128,
    top_index: usize,
    ...
    let _ = path;
    let row_index = source.row_index;
    let domain_len = source.domain_len;
    if top_index >= domain_len {
        return Err(Error::InvalidPcsOpen(
            "BaseFold terminal core path shape is invalid".to_string(),
        ));
    }
    let top_key = RaaSection5RelationTerminalValueKey {
        round: 0,
        row_index,
        index: top_index,
    };
    required_openings.insert(top_key, ());
    let mut current_value = section5_relation_terminal_opening_value(openings, top_key)?;
    let mut current_index = top_index;
    for (round, &challenge) in challenges.iter().enumerate() {
        ...
        let sibling_key = RaaSection5RelationTerminalValueKey {
            round,
            row_index,
            index: sibling_index,
        };
        required_openings.insert(sibling_key, ());
        let sibling_value = section5_relation_terminal_opening_value(openings, sibling_key)?;
    ...
    if current_index != 0 || current_value != claim {
        return Err(Error::InvalidPcsOpen(
            "BaseFold terminal core path does not fold to the claimed value".to_string(),
        ));
    }
    Ok(())
}

fn section5_relation_terminal_opening_value(
    openings: &BTreeMap<RaaSection5RelationTerminalValueKey, B128>,
    key: RaaSection5RelationTerminalValueKey,
) -> Result<B128, Error> {
    openings.get(&key).copied().ok_or_else(|| {
        Error::InvalidPcsOpen("RAA Section 5 terminal opening value is missing".to_string())
    })
}

fn basefold_terminal_core_value_after_round(
    path: &BaseFoldTerminalCorePath,
    source: BaseFoldTerminalCoreRowSource,
    challenges: &[B128],
    target_round: usize,
    ...
    if target_round > challenges.len() {
        return Err(Error::InvalidPcsOpen(
            "BaseFold terminal core folded layer round is invalid".to_string(),
        ));
    }
    let _ = path;
    let row_index = source.row_index;
    let domain_len = source.domain_len;
    let mut current_value = section5_relation_terminal_opening_value(
        openings,
        RaaSection5RelationTerminalValueKey {
            round: 0,
            row_index,
    ...
    let mut current_index = top_index;
    for (round, &challenge) in challenges.iter().take(target_round).enumerate() {
        let active_len = domain_len >> round;
        let half = active_len >> 1;
        let sibling_index = if current_index < half {
            current_index + half
        ...
        let sibling_value = section5_relation_terminal_opening_value(
            openings,
            RaaSection5RelationTerminalValueKey {
                round,
                row_index,
...
fn section5_relation_terminal_base_authentication_queries(
    ...
    for (_path, &top_index) in paths.iter().zip(top_indices) {
        if top_index >= domain_len {
            return Err(Error::InvalidPcsOpen(
                "RAA Section 5 terminal base path index is invalid".to_string(),
            ));
        }
        let top_value = section5_relation_terminal_opening_value(
            openings,
            RaaSection5RelationTerminalValueKey {
                round: 0,
                row_index,
        ...
        queries.push((row_offset + top_index, top_value));
        if domain_len > 1 {
            ...
            queries.push((row_offset + sibling_index, sibling_value));
        }
    }
    Ok(queries)
}
...
fn section5_relation_terminal_folded_layer_queries(
    ...
    let mut queries = Vec::with_capacity(paths.len() * 2);
    let source = BaseFoldTerminalCoreRowSource {
        row_index,
        domain_len,
    };
    for (path, &top_index) in paths.iter().zip(top_indices) {
        let (current_index, current_value) = basefold_terminal_core_value_after_round(
            path,
            source,
            &check.challenges,
            round,
        ...
        let sibling_value = section5_relation_terminal_opening_value(
            ...
            RaaSection5RelationTerminalValueKey {
                round,
                row_index,
                index: expected_sibling_index,
            },
        )?;
        queries.push((current_index, current_value));
        queries.push((expected_sibling_index, sibling_value));
    }
    Ok(queries)
}
...
fn collect_backend_query_set_from_prover_state<H: Hash>(
    ...
    section5_relation_residual: Option<&RaaSection5RelationResidualQueryProof<H>>,
    section5_relation_terminal_core: &BaseFoldTerminalCoreAuthentication<H>,
    ...
    if let (Some(oracle), Some(proof), Some(relation), Some(auxiliary)) = (
        ...
        section5_relation_residual_queries.extend(proof.terminal_residual_authentication_queries(
            ...
            &public,
            schedule,
            &section5_relation_terminal_core.folded_layers,
        )?);
    }
    let section5_residual_public_for_helper = state
        .section5_relation_residual
        .as_ref()
        .map(|oracle| oracle.public());
    let section5_helper_public_for_helper = state
        .section5_permutation_helper
        .as_ref()
        .map(|oracle| oracle.public());
    let section5_permutation_helper_queries = match (
        ...
        (Some(_oracle), Some(proof)) => proof.authentication_queries(
            schedule,
            section5_residual_public_for_helper.as_ref(),
            section5_relation_residual,
            section5_helper_public_for_helper.as_ref(),
        )?,
        (None, None) => {
            let expected_count = section5_permutation_helper_absent_expected_count(
                schedule,
...
fn build_section5_permutation_helper_authentication_queries(
    section5_residual_len: usize,
    proof_queries: &[BackendProofQuery],
) -> Result<Vec<BackendProofQuery>, Error> {
    if section5_residual_len == 0 {
        return Ok(Vec::new());
    }
    if section5_residual_len % RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT != 0 {
        return Err(Error::InvalidPcsParam(
            "Section 5 residual query domain is not row-aligned".to_string(),
        ));
    }
    let permutation_len = section5_residual_len / RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT;
    if permutation_len == 0 || !permutation_len.is_power_of_two() {
        return Err(Error::InvalidPcsParam(
            "Section 5 permutation residual domain must be a non-empty power of two".to_string(),
        ));
    }
    ...
    for query in proof_queries {
        if query.domain != BackendProofQueryDomain::Section5RelationResidual
            || query.index >= permutation_len
        {
            continue;
        }
        for helper_index in
            raa_section5_permutation_residual_helper_indices(permutation_len, query.index)?
        {
            queries.push(BackendProofQuery {
                domain: BackendProofQueryDomain::Section5PermutationHelper,
                index: helper_index,
...
fn verify_section5_permutation_helper_query_proof<H: Hash>(
    ...
    let expected_count = match public {
        Some(public) => section5_permutation_helper_expected_query_count(
            public,
            residual_public,
            residual_proof,
            schedule,
        )?,
        None => section5_permutation_helper_absent_expected_count(schedule, residual_proof),
    };
    match (public, proof, expected_count) {
        ...
        (Some(public), Some(proof), _) => {
            proof.verify_schedule(public, residual_public, residual_proof, schedule)
        }
...
fn verify_section5_permutation_helper_residual_bindings<H: Hash>(
    public: Option<&RaaSection5PermutationHelperPublicCommitment<H>>,
    residual_public: Option<&RaaSection5RelationResidualPublicCommitment<H>>,
    proof: Option<&RaaSection5PermutationHelperQueryProof<H>>,
    residual_proof: Option<&RaaSection5RelationResidualQueryProof<H>>,
    ...
    let Some(residual_proof) = residual_proof else {
        return Ok(());
    };
    let public = public.ok_or_else(|| {
        Error::InvalidPcsOpen(
            "Section 5 residual-local helper binding requires a helper commitment".to_string(),
        )
    })?;
    let proof = proof.ok_or_else(|| {
        Error::InvalidPcsOpen(
            "Section 5 residual-local helper binding requires helper openings".to_string(),
        )
    })?;
    ...
    for residual_query in &residual_proof.queries {
        if residual_query.logical_index >= len {
            continue;
        }
        let expected_indices =
            raa_section5_permutation_residual_helper_indices(len, residual_query.logical_index)?;
        let mut helper_openings = Vec::with_capacity(expected_indices.len());
        for expected_index in expected_indices {
            let scheduled = scheduled_helper_authentication.next().ok_or_else(|| {
                Error::InvalidPcsOpen(
                    "Section 5 helper residual-local authentication query is missing from schedule"
                        .to_string(),
                )
            })?;
            if scheduled.domain != BackendProofQueryDomain::Section5PermutationHelper
                || scheduled.index != expected_index
            {
                return Err(Error::InvalidPcsOpen(
                    "Section 5 helper residual-local authentication query does not match schedule"
                        .to_string(),
                ));
            }
        ...
        let derived_residual = raa_section5_permutation_residual_from_helper_openings(
            len,
            gamma,
            residual_query.logical_index,
            &helper_openings,
        )?;
        if derived_residual != residual_query.value {
            return Err(Error::InvalidPcsOpen(
                "Section 5 helper local openings do not match permutation residual".to_string(),
            ));
        }
    }
    if scheduled_helper_authentication.next().is_some() {
        return Err(Error::InvalidPcsOpen(
            "too many Section 5 helper residual-local authentication queries".to_string(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn explicit_schedule(layout: &SystematicAugmentedRfcLayout) -> HolographicQuerySchedule {
        HolographicQuerySchedule {
            ...
            raa_auxiliary_queries: Vec::new(),
            raa_auxiliary_authentication_queries: Vec::new(),
            section5_permutation_helper_authentication_queries: Vec::new(),
        }
    }
    ...
    fn backend_section5_relation_strategy_checks_residual_oracles() {
        ...
        let initial_helper_authentication_query_count =
            helper_schedule.section5_permutation_helper_authentication_query_count();
        let initial_helper_query_proof_count =
            helper_schedule.section5_permutation_helper_query_proof_count();
        ...
        helper_schedule.section5_permutation_helper_authentication_queries =
            build_section5_permutation_helper_authentication_queries(
                params.spec().praa.praa_codeword_len * RAA_SECTION5_RELATION_RESIDUAL_ROW_COUNT,
                &helper_schedule.proof_queries,
            )
            .unwrap();
        assert_eq!(
            helper_schedule.section5_permutation_helper_authentication_query_count(),
            initial_helper_authentication_query_count + 12,
            "permutation residual row 0 schedules the four product-tree local triples"
        );
        assert_eq!(
            helper_schedule.section5_permutation_helper_query_proof_count(),
            initial_helper_query_proof_count + 13
        );
        ...
        let expected_helper_query_count = section5_permutation_helper_expected_query_count(
            prequery.section5_permutation_helper.as_ref().unwrap(),
            prequery.section5_relation_residual.as_ref(),
            helper_proof.section5_relation_residual.as_ref(),
            &helper_schedule,
        )
        .unwrap();
        assert_eq!(helper.query_count(), expected_helper_query_count);
        ...
        let err = params
            ...
            .unwrap_err();
        let err = format!("{err:?}");
        assert!(err.contains("Merkle") || err.contains("terminal"));

        let mut duplicate_terminal_opening = proof.clone();
        let terminal = duplicate_terminal_opening
            .section5_relation_residual
            .as_mut()
            .unwrap()
            .terminal_proof
            .as_mut()
            .unwrap();
        terminal.openings.push(terminal.openings[0].clone());
        let err = params
            .verify_query_proof(
                &prequery,
                &request,
                &schedule,
                &duplicate_terminal_opening,
                &top_queries,
            )
            .unwrap_err();
        assert!(format!("{err:?}").contains("canonical"));

        let mut missing_terminal_opening = proof.clone();
        missing_terminal_opening
            .section5_relation_residual
            .as_mut()
            .unwrap()
            .terminal_proof
        ...
        tampered_helper_relation
            ...
            .as_mut()
            .unwrap()
            .queries[derived_helper_offset]
            .value += B128::ONE;
        let relation_challenges = squeeze_raa_section5_relation_challenges(
            params.spec(),
            &prequery.compiler_parity,
            prequery.auxiliary.as_ref(),
            &request,
        );
        let err = verify_section5_permutation_helper_residual_bindings(
            prequery.section5_permutation_helper.as_ref(),
            prequery.section5_relation_residual.as_ref(),
            tampered_helper_relation
                .section5_permutation_helper
                .as_ref(),
            tampered_helper_relation.section5_relation_residual.as_ref(),
            &helper_schedule,
            relation_challenges.gamma,
        )
        .unwrap_err();
        assert!(format!("{err:?}").contains("permutation residual"));
