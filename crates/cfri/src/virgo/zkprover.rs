...
use crate::pip_fri::util::interpolate_vecs_value::{InterpolateVecsValue, QueryVecsResult};
...
use crate::pip_fri::util::{
    fiat_shamir::RandomOracle, helper::Helper, merkle_tree::MerkleTreeProver,
};
use crate::pip_fri::util::{CODE_RATE, SECURITY_BITS};
...
impl<T: PrimeField> ZKFriProver<T> {
    pub fn new(
        ...
        let poly_u = UnivariatePolynomial::from_coefficients_slice(&coeffs_u)
            + poly_delta.mul_by_vanishing_poly(*vector_interpolation_coset);
        let function_u = fri_cosets[0].fft(poly_u.coeffs());

        let poly_s = UnivariatePolynomial::rand(
            2 * coeffs_u.len() + SECURITY_BITS / CODE_RATE - 1,
            &mut rng,
        );
    ...
    pub fn commit_functions(&mut self, verifier: &mut ZKFriVerifier<T>, open_point: &Vec<T>) {
        ...
        let poly_v = UnivariatePolynomial::from_coefficients_vec(
            self.vector_interpolation_coset.ifft(&public_vector),
        );
        assert!(poly_v.degree() < self.vector_interpolation_coset.size());
        let poly_uv_s = &(&self.poly_u * &poly_v) + &(&self.poly_s * self.oracle.rlc);
        let (h, remainder) = poly_uv_s
            .divide_by_vanishing_poly(self.vector_interpolation_coset)
            .unwrap();
        // assert!(h.degree() < self.vector_interpolation_coset.size());
        assert_eq!(
            remainder.degree(),
            self.vector_interpolation_coset.size() - 1
        );
        ...
        let evaluation_s = self
            .poly_s
            .evaluate_over_domain_by_ref(self.vector_interpolation_coset)
            .evals
            .iter()
            .sum();
    ...
    pub fn query(
        ...
        for i in 0..self.total_round {
            ...
            if i == 0 {
                function_us_res = Some(self.function_us.query(&leaf_indices));
        ...
        (
            folding_res,
            function_us_res.unwrap(),
            function_h_res.unwrap(),
            v_value.unwrap(),
        )
    }
}
