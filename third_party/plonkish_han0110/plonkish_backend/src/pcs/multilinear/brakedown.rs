...
mod test {
    use crate::{
        pcs::{
            ...
            test::{
                ...
                run_commit_open_rejects_tampered_proof, run_commit_open_verify,
            },
    ...
    #[test]
    fn rejects_wrong_public_evaluation() {
        let k = 2;
        let poly_size = 1 << k;
        let param = Pcs::setup(poly_size, 1, OsRng).unwrap();
        let (pp, vp) = Pcs::trim(&param, poly_size, 1).unwrap();
        let poly = <<Pcs as PolynomialCommitmentScheme<Fr>>::Polynomial as Polynomial<Fr>>::rand(
            poly_size, OsRng,
        );
        ...
        assert_eq!(verify(eval), Ok(()));
        assert!(verify(eval + Fr::ONE).is_err());
    }
}
