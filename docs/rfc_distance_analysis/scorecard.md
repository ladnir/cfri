...
| Tau-one carried quotient-line state | 0 | 0 | 0 | open | Current proof target | `rfc_tau1_carried_line_state.md` identifies the needed state: carry a marked root-compatible quotient line `R <= E_A(V/L)` through the child diagram. The first target is `(4,7)>=(2,8)`, whose dominant child-table row has `outer_tau1_charged_postroot_qdim=3`. Expanded diagnostics show this equals the invisible-fiber budget, so carrying only the root-visible image saves zero; the proof must carry the full ambient line or pay the new fiber dimension. |
