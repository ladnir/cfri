...
\begin{enumerate}[leftmargin=0.24in,label=\textbf{S\arabic*.},itemsep=0.08em,topsep=0.1em,parsep=0pt]
  ...
  \item \(\verifier\): \(b\gets\mathcal{F}_{\mathrm{PCS}}.\mathsf{Eval}(C,r,u)\).
...
\]
Only after these endpoint requests are authenticated by the existing
\(\mathcal F_{\mathrm{MLIOP}}\), \(\mathcal F_{\mathrm{MLEAuth}}\), or PCS
interface does the sumcheck terminal claim become meaningful.
