...
impl Permutation {
    ...
    fn repeat_interleave_long<F: BlazeField>(
        ...
        for vec in input {
            let repetition = repetition_code(vec, rate);
            let mut new_input = vec![F::zero(); repetition.len()];
            new_input.par_iter_mut().enumerate().for_each(|(i, mut x)| {
                let y = self.permutation1[i];
                *x = repetition[y as usize];
            });
            new_inputs.push(new_input);
        }
...
fn test_puncture() {
    ...
    let perm = Permutation::create(&mut rng, 32);
    let a = F { value: 8u64 };
    let input = vec![a; 32];
    let result = perm.puncture(input);
    assert_eq!(result, vec![a; 16]);
}
...
fn test_permutation() {
    ...
    assert_eq!(
        permuted_input
            ...
            .map(|x| x.get_value() as usize)
            .collect::<Vec<_>>(),
        p.permutation1.iter().map(|i| i / 2).collect::<Vec<_>>()
    );
}
...
#[test]
fn test_accumulator() {
    let mut serial = Blazeu64::rand_vec(1 << 8);
    let mut parallel = serial.clone();
    serial_accumulator(&mut serial);
    parallel_accumulator(&mut parallel);
...
fn test_encode_bits_long_64(k: usize, col_size: usize) {
    ...
    let mut rng = ChaCha8Rng::seed_from_u64(k as u64);
    let p1 = Permutation::create(&mut rng, 1 << k);
    let p2 = Permutation::create(&mut rng, 1 << k);
    ...
    assert_eq!(codeword.len(), col_size);
...
#[test]
fn test_encode_long() {
    test_encode_bits_long_64(6, 2);
}

#[test]
fn test_repeat_interleave_applies_permutation() {
    let p = Permutation {
        permutation1: vec![2, 0, 3, 1],
        permutation2: vec![0, 1, 2, 3],
        permutation3: vec![0, 1, 2, 3],
        puncturing: HashMap::new(),
    };
    let input = vec![Blazeu64 { value: 10 }, Blazeu64 { value: 20 }];

    assert_eq!(
        p.repeat_interleave(input, 2),
        vec![
            Blazeu64 { value: 20 },
            Blazeu64 { value: 10 },
            Blazeu64 { value: 20 },
...
#[test]
fn test_encode_bits_ser_is_linear() {
    ...
    let b = (0..8)
        ...
        .collect::<Vec<_>>();
    let a_xor_b = a.iter().zip(&b).map(|(x, y)| *x ^ *y).collect::<Vec<_>>();
    let p = Permutation::create(&mut rng, a.len() * rate);

    let enc_a = encode_bits_ser(a, &p, rate);
    let enc_b = encode_bits_ser(b, &p, rate);
    let enc_sum = encode_bits_ser(a_xor_b, &p, rate);
    let sum_encs = enc_a
        .iter()
        .zip(&enc_b)
        .map(|(x, y)| *x ^ *y)
