...
The packed RAA code has parameters
\[
  (k,r,\pi_1,\pi_2),
  \qquad N=kr,
\]
where $k$ is the message length, $r$ is the repetition rate, $N$ is the codeword
length, and $\pi_1,\pi_2$ are permutations of $\{0,\ldots,N-1\}$.
The parameter $r$ says how many repeated slots each message coordinate receives
before the first permutation.  If there were no permutation, the length-$N$
repetition row would be
\[
  m[0],\ldots,m[0],
  m[1],\ldots,m[1],
  \ldots,
  m[k-1],\ldots,m[k-1],
\]
with exactly $r$ copies of each $m[a]$.  Thus a repeated-row index $j$ maps back
to its source message coordinate by $\lfloor j/r\rfloor$.

For a message $m\in\F^k$, the codeword $c=\RAA(m)\in\F^N$ is defined through
three auxiliary rows $u_2,u_3,u_4$:
\[
  u_2[i] = m[\lfloor \pi_1[i]/r \rfloor],
\]
\[
...
\]
The name RAA reflects the shape ``repeat, accumulate, accumulate,'' with
permutations inserted to spread local structure.

\subsection{Why These Rows Are Useful}

The definitions above contain global prefix sums.  A verifier cannot afford to
recompute every prefix sum.  The auxiliary rows turn the global statement into
local recurrences:
\[
...
\[
  c[i]-c[i-1]-u_4[i]=0,
\]
with the convention that $u_3[-1]=c[-1]=0$.  These equations can be checked at
sampled positions.

The repetition and permutation parts are checked by additional local residuals
and product-tree helper witnesses.  The verifier does not read the whole trace;
it samples positions and uses random batching to make a cheating prover unlikely
to hide a nonzero error.

\subsection{How The Permutation Proof Works}

The RAA definition uses two public permutations.  The first one hides the
repetition pattern:
\[
  u_2[i]=m[\lfloor\pi_1[i]/r\rfloor].
\]
Equivalently, if we define an implicit row $u_1$ by
\[
  u_1[\pi_1[i]]=u_2[i],
\]
then $u_1$ should be constant on every length-$r$ repetition block.  The second
permutation says
\[
  u_4[i]=u_3[\pi_2[i]].
\]
So the verifier must be convinced that the prover did not invent unrelated
values for $u_2,u_3,u_4$.

For a tiny example, suppose a prover claims that two indexed lists are the same
after permutation:
\[
...
commits to product trees for $f_1,g_1,f_2,g_2$.  A product tree stores the leaves
at the bottom, each parent as the product of two children, and the final product
at the root.  The verifier checks sampled parent equations and root equalities:
\[
  \mathsf{root}(f_1)=\mathsf{root}(g_1),
  \qquad
  \mathsf{root}(f_2)=\mathsf{root}(g_2).
\]
Sampled helper leaves are also bound back to the auxiliary rows, so a prover
cannot use a correct product tree for the wrong leaves.  The later RAA residual
rows turn these sampled tree checks and root equalities into local error rows
that can be batched with the accumulator and repetition checks.

\subsection{Linearity}

Packed RAA is linear.  Each step is a linear operation: copying a message entry,
taking prefix sums, permuting a flat vector, and taking prefix sums again.  Therefore
