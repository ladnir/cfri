...
column order and is decoded least-significant-bit first by `scripts/rfc_distance_analysis/describe_rfc_columns.py`; the
...
```text
python scripts/rfc_distance_analysis/describe_rfc_columns.py \
  --systematic \
  --depth 2 \
  --parity-expansion 7 \
  --columns 0:1:4:18
