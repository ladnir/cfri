...
impl<F: Field, C: Circuit<F>> PlonkishCircuit<F> for Halo2Circuit<F, C> {
    fn circuit_info_without_preprocess(
        &self,
    ) -> Result<PlonkishCircuitInfo<F>, crate::plonkish_backend::Error> {
        let Self {
            k,
            instances,
            cs,
    ...
    fn circuit_info(&self) -> Result<PlonkishCircuitInfo<F>, crate::plonkish_backend::Error> {
        let Self {
            k,
            instances,
            cs,
        ...
        .map_err(|err| {
            crate::plonkish_backend::Error::InvalidSnark(format!("Synthesize failure: {err:?}"))
        })?;
    ...
    fn synthesize(
        ...
        phase: usize,
        challenges: &[F],
    ) -> Result<Vec<Vec<F>>, crate::plonkish_backend::Error> {
