...
macro_rules! izip_eq {
    ...
    (@closure $p:pat => ($($tup:tt)*) , $_iter:expr $(, $tail:expr)*) => {
        $crate::plonkish_backend::util::izip_eq!(@closure ($p, b) => ($($tup)*, b) $(, $tail)*)
    };
    ($first:expr $(,)*) => {
        itertools::__std_iter::IntoIterator::into_iter($first)
    };
    ($first:expr, $second:expr $(,)*) => {
        $crate::plonkish_backend::util::izip_eq!($first).zip_eq($second)
    };
    ($first:expr $(, $rest:expr)* $(,)*) => {
        $crate::plonkish_backend::util::izip_eq!($first)
            $(.zip_eq($rest))*
            .map($crate::plonkish_backend::util::izip_eq!(@closure a => (a) $(, $rest)*))
    };
}
...
pub mod test {
    use crate::plonkish_backend::util::arithmetic::Field;
    use rand::{
        rngs::{OsRng, StdRng},
